...
- [x] **G2. Staff module caps** — `Membership.staffModuleAllowList` + `UserInvite.staffModuleAllowList`; `computeEffectiveModules`; `requireModuleForSession`; sidebar + staff APIs/actions use **`effectiveModules`**; admin invite/add-member optional checklists. Seed: use **`Membership.staffModuleAllowList`** on any STAFF user (e.g. on `synthetic-test` after editing in admin).
