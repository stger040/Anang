...
| Auth | **Manual bearer** (`GREENWAY_FHIR_ACCESS_TOKEN`) for early probes, **or** **client-credentials OAuth** (`GREENWAY_FHIR_CLIENT_ID`, `GREENWAY_FHIR_TOKEN_URL`, optional `GREENWAY_FHIR_OAUTH_SCOPE`) with backend-service `private_key_jwt` (`GREENWAY_FHIR_CLIENT_PRIVATE_KEY`, optional `GREENWAY_FHIR_CLIENT_KEY_ID`) or legacy `GREENWAY_FHIR_CLIENT_SECRET` — resolved automatically by Implementation hub test + cron probe when static token is unset. |
