...
describe("assemble837pProfessional", () => {
  ...
  it("preserves alphanumeric HCPCS codes in SV1 (does not strip letters)", () => {
    const input: Assemble837pInput = {
      now: new Date("2026-03-29T12:00:00.000Z"),
      claimControlNumber: "HCPCS-CLAIM-01",
      controls: { isa13: "000000001", gs06: "1", st02: "0001" },
      tradingPartner: defaultTradingPartnerFor837p(),
      patient: {
        firstName: "Jane",
    ...
    expect(x12).toContain("SV1*HC:J1100*25.00*UN*1***1~");
    expect(x12).toContain("SV1*HC:E0601:NU*400.00*UN*1***1~");
    // Digits-only sanitization would corrupt these to 1100 / 0601.
    expect(x12).not.toContain("SV1*HC:1100*");
    expect(x12).not.toContain("SV1*HC:0601");
  });
