...
const CREATE_CASE_NUMBER_RETRIES = 3;
...
export async function createPriorAuthCaseDb(args: {
  ...
  for (let attempt = 0; attempt < CREATE_CASE_NUMBER_RETRIES; attempt += 1) {
    try {
      ...
      if (isUniquePriorAuthCaseNumberError(error) && attempt < CREATE_CASE_NUMBER_RETRIES - 1) {
        continue;
      }
      throw error;
    }
  ...
  throw new Error("Unable to allocate prior authorization case number");
}
...
export async function updatePriorAuthCaseStatusDb(args: {
  ...
  const row = await db.$transaction(async (tx) => {
    ...
    if (updated.count !== 1) {
      throw new Error("Case status changed; reload and retry");
    }
...
function isUniquePriorAuthCaseNumberError(error: unknown): boolean {
