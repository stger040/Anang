...
const CASE_NUMBER_RETRY_ATTEMPTS = 5;
...
function isCaseNumberConflict(error: unknown): boolean {
  ...
  return (
    ...
    error.code === "P2002" &&
    targetText.includes("tenantId") &&
    targetText.includes("caseNumber")
  );
}

async function runPriorAuthWrite<T>(
  db: PrismaClient,
  fn: (tx: PriorAuthDb) => Promise<T>,
): Promise<T> {
  const maybeTransactional = db as PrismaClient & {
    $transaction?: (callback: (tx: Prisma.TransactionClient) => Promise<T>) => Promise<T>;
  };

  if (typeof maybeTransactional.$transaction === "function") {
    return maybeTransactional.$transaction(fn);
  }
...
export async function createPriorAuthCaseDb(args: {
  ...
  for (let attempt = 0; attempt < CASE_NUMBER_RETRY_ATTEMPTS; attempt += 1) {
    const caseNumber = await nextPriorAuthCaseNumber(db, tenantId);

    try {
      const row = await runPriorAuthWrite(db, async (tx) => {
      ...
      if (attempt < CASE_NUMBER_RETRY_ATTEMPTS - 1 && isCaseNumberConflict(error)) {
        continue;
      }
      throw error;
    }
