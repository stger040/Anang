...
type PriorAuthNumberingDb = Pick<PrismaClient, "priorAuthCase"> | Prisma.TransactionClient;

export async function nextPriorAuthCaseNumber(
  db: PriorAuthNumberingDb,
  tenantId: string,
): Promise<string> {
...
function isCaseNumberConflict(error: unknown): boolean {
  ...
  return typeof target === "string" && target.includes("caseNumber");
}
...
export async function createPriorAuthCaseDb(args: {
  ...
  for (let attempt = 0; attempt < CASE_NUMBER_RETRY_LIMIT; attempt += 1) {
    try {
      ...
      if (attempt < CASE_NUMBER_RETRY_LIMIT - 1 && isCaseNumberConflict(error)) {
        continue;
      }
      throw error;
    }
