...
describe("createPriorAuthCaseDb", () => {
  ...
  it("rejects a claim that belongs to another patient", async () => {
    const db = {
      patient: { findFirst: vi.fn().mockResolvedValue({ id: "p1" }) },
    ...
    await expect(
      createPriorAuthCaseDb({
        ...
        tenantId: "t1",
        session,
        input: { patientId: "p1", claimId: "claim-b", payerName: "Payer A" },
      }),
    ).rejects.toThrow("Claim not found for patient");
    expect(db.claim.findFirst).toHaveBeenCalledWith({
      where: { id: "claim-b", tenantId: "t1", patientId: "p1" },
    });
  });
});
...
describe("linkPriorAuthToClaimDb", () => {
  ...
  it("rejects cross-patient claim links", async () => {
    const db = {
      priorAuthCase: {
        findFirst: vi.fn().mockResolvedValue({
          id: "case1",
