...
export default async function StatementDetailPage({
  ...
  const canViewConnect = ctx.effectiveModules.has(ModuleKey.CONNECT);
  const priorAuthForClaim = stmt.claim && canViewConnect
    ...
    <div className="space-y-8">
      ...
      {stmt.claim ? (
        <Card className="border-slate-200 bg-slate-50/80 p-5">
          ...
          {canViewConnect && priorAuthForClaim.length ? (
